...
class Model:
    ...
    def build_model(self, dataset, nb_classes=2):
        ...
        self.model.add(layers.Conv2D(32, (3, 3),activation='relu'))  # 32维卷积层
        #self.model.add(Activation('relu'))  # 4 激活函数层

        self.model.add(layers.MaxPooling2D(pool_size=(2, 2)))  # 5 池化层
        self.model.add(layers.Dropout(0.25))  # 6 Dropout层

        self.model.add(layers.Conv2D(64, (3, 3),activation='relu',  padding='same'))  # 7  2维卷积层
        #self.model.add(Activation('relu'))  # 8  激活函数层

        self.model.add(layers.Conv2D(64, (3, 3),activation='relu'))  # 9  2维卷积层
        #self.model.add(Activation('relu'))  # 10 激活函数层

        self.model.add(layers.MaxPooling2D(pool_size=(2, 2)))  # 11 池化层
        self.model.add(layers.Dropout(0.25))  # 12 Dropout层

        self.model.add(layers.Flatten())  # 13 Flatten层
        self.model.add(layers.Dense(512))  # 14 Dense层,又被称作全连接层
        self.model.add(layers.Activation('relu'))  # 15 激活函数层
        self.model.add(layers.Dropout(0.5))  # 16 Dropout层
        self.model.add(layers.Dense(nb_classes))  # 17 Dense层
        self.model.add(layers.Activation('softmax'))  # 18 分类层，输出最终结果
