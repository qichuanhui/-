...
def get_tri_pt_index_list(landmarks_points):
    ...
    subdiv.insert(landmarks_points)#导入需要三角划分的点
    triangles = subdiv.getTriangleList()#得到三角坐标点
   ...
   #把三角形坐标和68个特征点坐标对应起来
        ...
        #因为所匹配的点可能在68个点之外，因此加一个判断条件滤去这些点
...
def get_one_rect_from_tri(img,tri,landmarks):
    ...
    (x,y,w,h) = crop_rect#左上角坐标以及宽和高
    
    # 计算三个顶点在外接矩形上的坐标(把外接矩形单独拿出来，坐标改变，需要重新计算)
...
def get_face_cover(img_src,img_dst,landmarks_src,landmarks_dst,list_index_tris):        
    ...
    for tri in list_index_tris:
        ...
        M = cv2.getAffineTransform(pts_src,pts_dst)#转换矩阵，实现源图像替换目标图像
        ...
        cv2.fillConvexPoly(mask_dst,points_in_rect_dst,255)#只保留脸部矩形内的图像
        warped_tri = cv2.bitwise_and(warped_src,warped_src,mask=mask_dst)#与操作，进行叠加
...
if __name__ == "__main__":
    ...
    # 打开图片
    img_dst = cv2.imread('dong.jpg')
    img_src = cv2.imread('wuyanzu.jpg')
